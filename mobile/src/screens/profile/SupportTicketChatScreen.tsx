...
import type { SupportTicketStatus, SupportTicketThread } from '../../data/types';

type Props = NativeStackScreenProps<RootStackParamList, 'SupportTicketChat'>;
...
export function SupportTicketChatScreen({ route }: Props) {
  const colors = useThemeColors();
  const styles = useMemo(() => createStyles(colors), [colors]);
  const { ticketId } = route.params;
  const { getSupportTicketThread, replyToSupportTicket } = useApp();
